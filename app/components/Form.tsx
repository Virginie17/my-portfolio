  ...
  return (
    <div>
      <form ref={contactForm} onSubmit={handleSubmit} className="flex flex-col w-full p-3 pt-1 gap-1 box-border md:p-[30px_15px_15px_10px] xl:p-[25px_20px_20px_25px] xl:gap-2.5">
        <label className="text-secondary-color text-sm">{t('form.name')}</label>
        ...
        <label className="text-secondary-color text-sm">{t('form.email')}</label>
        ...
        <label className="text-secondary-color text-sm">{t('form.message')}</label>
        ...
        <input type="submit" value={t('form.send')} className="mt-3 p-1 w-[40%] self-center bg-tertiary-color rounded-[10px] border border-tertiary-color text-secondary-color text-sm cursor-pointer transition-transform duration-300 ease-in-out hover:scale-110 md:mt-5 xl:p-1.5 xl:w-[30%]" />
        {isSent && <p className="text-green-500 text-center text-sm">{t('form.successMessage')}</p>}
      </form>
    </div>
  );
};
